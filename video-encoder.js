(function(root){
  ...
  async function create(canvas,options){
    ...
    var source=new M.CanvasSource(canvas,{codec:codec,quality:quality,keyFrameInterval:2,latencyMode:'quality',onEncodedPacket:function(packet){bytes+=packet.byteLength;if(bytes>(options.memoryBudget||(globalThis.TypeDeformerRenderContext?globalThis.TypeDeformerRenderContext.budgets.videoBytes:512*1024*1024)))throw new Error('動画データが作業メモリ予算を超えました。長さまたはFrameサイズを調整してください。設定値は保持されています。');}});
