(function (root) {
  ...
  function source(g,L,fm,env,id){
    ...
    var result={canvas:tile.canvas,data:data,alpha:alpha,w:w,h:h,scale:scale,pad:pad,key:fingerprint(data),trace:env.traceContours};
  ...
  function entryFor(id,g,L,fm,env){
    var p=settings(id,g,env),s=source(g,L,fm,env,id),color=env.color(id),accent=env.params.accent||'#b52254',seed=(env.params.seed||17),key=[id,s.w,s.h,s.key,JSON.stringify(p),color,accent,seed,root.TypeDeformerRenderContext&&root.TypeDeformerRenderContext.current()?root.TypeDeformerRenderContext.current().factor:1,root.TypeDeformerRenderContext&&root.TypeDeformerRenderContext.current()?root.TypeDeformerRenderContext.current().purpose:'analysis'].join('|'),entry=cache.get(key);
    if(!entry){var result=renderers[id](s,p,rgb(color),rgb(accent),seed),bytes=root.TypeDeformerRenderContext?root.TypeDeformerRenderContext.byteSize(result):result.width*result.height*4;while(cacheBytes+bytes>MAX_BYTES&&cache.size){var first=cache.keys().next().value;cacheBytes-=cache.get(first).bytes;cache.delete(first);}entry={canvas:result,bytes:bytes};if(bytes<=MAX_BYTES){cache.set(key,entry);cacheBytes+=bytes;}}
  ...
  function bodyBounds(glyphs,bounds,fm,env){
    ...
    glyphs.forEach(function(g){boundedBodyIds.forEach(function(id){if(env.strength(g,id)<=.002||(g.opacity!=null&&g.opacity<=.002))return;
      ...
      [[b[0],b[1]],[b[2],b[1]],[b[2],b[3]],[b[0],b[3]]].forEach(function(q){var x=(g.x-s.pad+q[0]/s.scale-g.ox)*g.scaleX,y=(g.y-s.pad+q[1]/s.scale-g.oy)*g.scaleY,u=x+Math.tan((g.skewX||0)*Math.PI/180)*y,v=y+Math.tan((g.skewY||0)*Math.PI/180)*x,a=(g.rot||0)*Math.PI/180,X=g.ox+g.tx+u*Math.cos(a)-v*Math.sin(a),Y=g.oy+g.ty+u*Math.sin(a)+v*Math.cos(a);minX=Math.min(minX,X);maxX=Math.max(maxX,X);minY=Math.min(minY,Y);maxY=Math.max(maxY,Y);});
  ...
  function render(id,ctx,glyphs,width,height,pixelScale,L,fm,env){if(!renderers[id])return false;
    ...
    glyphs.forEach(function(g){var strength=env.strength(g,id);if(strength<.002||(g.opacity!=null&&g.opacity<.002))return;
      ...
      ctx.save();ctx.setTransform(pixelScale,0,0,pixelScale,0,0);ctx.translate(L.dx,L.dy);ctx.scale(L.s,L.s);ctx.translate(g.ox+g.tx,g.oy+g.ty);ctx.rotate((g.rot||0)*Math.PI/180);ctx.transform(1,Math.tan((g.skewY||0)*Math.PI/180),Math.tan((g.skewX||0)*Math.PI/180),1,0,0);ctx.scale(g.scaleX,g.scaleY);ctx.translate(-g.ox,-g.oy);ctx.globalAlpha=strength*(g.opacity==null?1:g.opacity);ctx.drawImage(entry.canvas,g.x-s.pad,g.y-s.pad,s.w/s.scale,s.h/s.scale);ctx.restore();
    });
